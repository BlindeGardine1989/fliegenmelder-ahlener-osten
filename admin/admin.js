...
function showAdmin(){ loginBox.hidden = true; adminBox.hidden = false; }
...
function renderReports(reports) {
  ...
  for (const r of reports) {
    ...
    const statusClass = r.status === "approved" ? "" : r.status === "hidden" ? "red" : "warn";
    const card = document.createElement("article");
    card.className = "adminCard";
    card.innerHTML = `
      <h2>Meldung ${escapeHtml((r.public_id || r.id || "").slice(0, 8))}</h2>
      <p><span class="badge ${statusClass}">${escapeHtml(r.status || "pending")}</span> · öffentlich sichtbar: <strong>${r.visible ? "ja" : "nein"}</strong></p>
      <p><strong>Datum:</strong> ${formatDate(r.client_timestamp || r.created_at)}<br><strong>Ort:</strong> ${escapeHtml(r.address || "–")}<br><strong>Belastung:</strong> ${escapeHtml(r.severity || "–")}/5<br><strong>Seit:</strong> ${escapeHtml(r.since || "–")}<br><strong>Tageszeit:</strong> ${escapeHtml(r.time_of_day || "–")}</p>
  ...
  list.querySelectorAll("button[data-action]").forEach(button => {
    button.addEventListener("click", async () => {
      ...
      const patch = action === "approve" ? { status: "approved", visible: true } : action === "hide" ? { status: "hidden", visible: false } : { status: "pending", visible: false };
...
exportBtn?.addEventListener("click", async () => {
  ...
  for (const r of data) {
    rows.push([r.public_id || r.id || "", r.status || "", r.visible ? "ja" : "nein", r.client_timestamp || r.created_at || "", r.address || "", r.severity || "", r.since || "", r.time_of_day || "", r.note || "", r.contact_private || "", r.lat || "", r.lng || "", r.photo_path ? `${SUPABASE_URL}/storage/v1/object/public/report-photos/${r.photo_path}` : ""]);
  }
