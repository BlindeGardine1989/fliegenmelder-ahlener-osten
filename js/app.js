...
export async function loadPublicReports(){if(!IS_CONFIGURED||SUPABASE_PUBLISHABLE_KEY.includes("HIER_DEINEN")) return [];const{data,error}=await supabase.from("reports_public").select("*").order("created_at",{ascending:false});if(error){console.warn("Meldungen konnten nicht geladen werden:",error);return []}return data||[]}
