...
function render(){
  ...
  reports.filter(r => value === "all" || value === "4plus" && Number(r.severity)>=4 || Number(r.severity)===Number(value)).forEach(r => {
    const photo = r.photo_url ? `<img class="popupPhoto" src="${r.photo_url}" alt="Foto zur Meldung">` : "";
    L.marker([r.lat,r.lng], {icon:markerIcon(r.severity)}).addTo(layer).bindPopup(`<strong>${escapeHtml(r.address)}</strong><br>Belastung: ${r.severity}/5<br>Seit: ${escapeHtml(r.since || "Unklar")}<br>${photo}${r.note ? "<br>Bemerkung: "+escapeHtml(r.note) : ""}`);
  });
}
